...
One contract lint still cannot check — `useLoadProtocol`'s memoized-`perform` requirement — is enforced by a development-mode guard inside the protocol instead: the load effect counts its own firings and throws when the rate can only be the unmemoized-`perform` fetch loop (recorded in [[partial-load-rendering]]).
