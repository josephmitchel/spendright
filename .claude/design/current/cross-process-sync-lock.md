...
Two additions from the 2026-09-07 audit. The lock session raises its own `statement_timeout` above the 60s wait, because the pool-wide 30s default added under [[requests-have-deadlines]] would otherwise cancel the advisory wait early. The lock query also passes a per-query `query_timeout` override (75s): pg's client-side `query_timeout` is a construction-time JS timer that a runtime `SET` cannot raise, so without the override the pool's 35s timer killed the wait first with an unclassifiable plain `Error` — a generic 500 instead of the designed 503 `SYNC_LOCKED` (found by the 2026-09-07 audits, fixed 2026-09-07). The override keeps the server-side `lock_timeout` cancel first, which surfaces as `55P03` and maps to `SYNC_LOCKED`. And item deletion (`removeItemCompletely`, [[item-delete-plaid-first]]) now takes this lock too, so a delete cannot race a running sync.
