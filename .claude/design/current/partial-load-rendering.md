...
tags: [Promise.allSettled, HomeClient refresh, AccountView, loaded, itemsLoaded, Retry]
date: 2026-09-04
---

Loads use `Promise.allSettled` per endpoint, fetch and parse together. A read that failed leaves its slice of state exactly as it was; messages that assert what the database holds ("No institutions", "Card not supported", "No transactions") are gated on the read that supports them. Errors render on screen with a Retry that clears the message on click and never blanks content that did load. Console output is not a user surface.
