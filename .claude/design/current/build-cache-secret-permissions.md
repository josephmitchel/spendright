...
description: Turbopack's cache under .next persists env secrets into world-readable files, so .next is chmod'd go-rwx after every build (postbuild) and on every start (scripts/start.mjs); files written during a live dev session are an accepted attended-work residual
tags:
  [
...
Mitigation, confirmed 2026-09-06: `chmod -R go-rwx .next` runs around every build (`prebuild` creates-then-tightens the directory so its 0700 mode survives even a failed build — `postbuild` only fires on success and covers the files; `next build` cleans inside distDir without recreating it, so the directory mode stands), before every dev session (`predev` — otherwise `next dev` recreating a deleted `.next` would leave the directory itself 0755 under the default umask), and at every `npm run start` (scripts/start.mjs, fail-closed: a failed chmod there refuses to start the server). Residual: files Turbopack writes _while_ a `next dev` session is running are 0644 until the next build/dev/start re-tightens them, though the 0700 `.next` directory blocks traversal for as long as it stands — accepted because dev sessions are short-lived, attended work ([[non-local-request-guard]]). Rotating `ENCRYPTION_KEY` is warranted only if the directory was actually shared, and costs every stored token ([[access-tokens-encrypted]]).
