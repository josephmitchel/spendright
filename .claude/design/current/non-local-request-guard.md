...
- Clickjacking (2026-09-04 audit): every response carries `frame-ancestors 'none'` in the CSP (built per-request in src/proxy.ts since 2026-09-07, [[nonce-based-csp]]) plus `X-Frame-Options: DENY` from `next.config.ts` — a page iframing `http://localhost:3000` would produce same-origin clicks the Origin check cannot see.
