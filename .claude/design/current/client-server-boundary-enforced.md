...
Where a type is small and shared, it still lives in a dependency-free module instead of being type-imported out of the server graph at all: `CategoryKind` sits in `src/lib/category-kinds.ts`, joining `sync-messages.ts`, `plaid-errors.ts`, `public-error.ts`, `log.ts`, `http.ts`, `serialize.ts`, `api-paths.ts`, `pagination.ts`, `account-display.ts`, `sync-failure.ts`.
