...
- **Client fetches: 120s `AbortSignal.timeout`** (`REQUEST_TIMEOUT_MS` in `getJson`/`sendJson`, the one fetch chokepoint [[typed-api-contract]]). A request that never answers would leave `useLoadProtocol` unsettled forever — "Loading…" with the Retry withheld, a failure [[partial-load-rendering]] otherwise has no path for. Sized well past the slowest legitimate route (a multi-item sync-all with not-ready polling, [[not-ready-poll-budgets]]); the abort surfaces as a normal failed read with Retry. A fetch rejection (abort or network) throws the caller's fallback message, keeping the error-string allow-list intact ([[error-message-allow-list]]).

- **Postgres: pool-level timeouts + a row-lock cap in the sync transaction** (added 2026-09-07 — the 2026-09-07 reliability audit found this record's principle was never extended to the one remaining unbounded network dependency, where a stalled backend wedged `withItemSyncLock`'s held connection and the sync guards with no log line). `POOL_CONFIG` (src/lib/pool-config.ts since 2026-09-07, [[shared-pool-config]]): `connectionTimeoutMillis` 10s, `statement_timeout` 30s, `query_timeout` 35s (the server-side cancel wins, yielding a clean pg error; the one exception is the advisory-lock acquisition, whose 60s wait outlives the client-side timer — it passes a 75s per-query `query_timeout` override so the server-side cancel still wins there, [[cross-process-sync-lock]], fixed 2026-09-07); the seed and rotation scripts build their own pools from the same shared definition. `runSyncItem`'s transaction sets `local lock_timeout = '10s'` over its `for update` reads, matching the 3s cap the category PATCH already had — one write path no longer tolerates an unbounded lock wait the other rejects. The advisory-lock session raises its own `statement_timeout` above its 60s wait so the pool default cannot cancel it ([[cross-process-sync-lock]]).
