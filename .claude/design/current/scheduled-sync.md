---
name: scheduled-sync
description: Automatic sync is an in-process timer started once per server from src/instrumentation.ts — one run shortly after startup, then hourly; the scheduler and POST /api/sync share a single-flight sync-all runner whose guard lives on globalThis because the bundler duplicates the module across chunks; the manual Sync all button remains alongside
tags:
  [
    startSyncScheduler,
...
## Automatic sync (decided 2026-09-04)

Syncing should not depend on the user pressing a button. The intended design is that syncs are triggered automatically. The "Sync all" button and the inline sync at link time ([[link-flow]]) are acceptable alongside the automatic path. Built 2026-09-04 as a Plaid webhook path; replaced 2026-09-05 by the in-process scheduler below when the webhook's tunnel proved to be an exposure itself (retired [[webhook-triggered-sync]]).

## The in-process scheduler (confirmed 2026-09-05)
...
Per-item failures are recorded on `items.error` through the allow-list exactly as the manual path always did ([[error-message-allow-list]]). The scheduler starts from `register()` via dynamic import (the runtime bind assertion that used to be scheduled ahead of it is retired — [[runtime-bind-assertion]]), and a broken config (unset DATABASE_URL, say) stops the server with the error named via an explicit catch — Next itself does not treat a rejected `register()` as fatal (verified 2026-09-05), so the exit cannot be left implicit ([[config-validated-not-assumed]]). Under `next start`, instrumentation only runs on the first request, so `npm run start` goes through `scripts/start.mjs`, which sends a loopback warm-up request at startup — the scheduler starts when the server does, not when traffic first arrives. Fulfils the automatic-sync intent above; [[non-local-request-guard]] no longer carries any exemption. The home page reflects these background runs via [[home-reflects-background-sync]].
