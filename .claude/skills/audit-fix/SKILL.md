...
After you know what audit it is, navigate to that audit's folder in `.claude/snapshot/audits`. Each concern is its own `.md` file with YAML frontmatter — select the ones whose `level` matches what the user requested, skipping any with `status: resolved`. Familiarize yourself with those concerns, then come up with a plan to address every one of them. Ask the user any questions if you are unsure about what direction you should go with a change. Never assume, no stupid questions.
