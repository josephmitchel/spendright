...
// the transaction page: paging only re-reads transactions. The load effect
// lives in the protocol; `reload` re-runs the load loudly (Retry) and
// `refresh` re-runs it silently (the page's poll).
      ...
      (load: LoadReads<'account' | 'cards'>) =>
        load(
          ...
          (results) => {
            ...
            const accountBody =
              results.account.status === 'fulfilled' ? results.account.value : null;
